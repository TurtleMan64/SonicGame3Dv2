 ...
 *
 */
public class ShadowMapMasterRenderer2 
{
	private static final int SHADOW_MAP_SIZE = 2048*2;//HD = 2048*2

	private ShadowFrameBuffer2 shadowFbo;
	private ShadowShader2 shader;
	private ShadowBox2 shadowBox;
	private Matrix4f projectionMatrix = new Matrix4f();
	private Matrix4f lightViewMatrix = new Matrix4f();
	private Matrix4f projectionViewMatrix = new Matrix4f();
	private Matrix4f offset = createOffset();

	private ShadowMapEntityRenderer2 entityRenderer;

	/**
	 * Creates instances of the important objects needed for rendering the scene
	 * to the shadow map. This includes the {@link ShadowBox2} which calculates
	 ...
	 * {@link ShadowFrameBuffer2} to which the scene is rendered. The size of the
	 * shadow map is determined here.
	 * 
	 * @param camera
	 *            - the camera being used in the scene.
	 */
	public ShadowMapMasterRenderer2(Camera camera) 
	{
		shader = new ShadowShader2();
		shadowBox = new ShadowBox2(lightViewMatrix, camera);
		shadowFbo = new ShadowFrameBuffer2(SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
		entityRenderer = new ShadowMapEntityRenderer2(shader, projectionViewMatrix);
	}
	 ...
	 */
	public void render(ArrayList<Entity> entities, Light sun) 
	{
		shadowBox.update();
	 ...
	 * the "view cuboid" which was also calculated in the {@link ShadowBox2}
	 ...
	 *            "view cuboid".
	 */
	private void prepare(Vector3f lightDirection, ShadowBox2 box) 
	{
	 ...
	 * on the values that were calculated in the {@link ShadowBox2} class.
	 * 
	 * @param width
	 *            - shadow box width.
	 * @param height
