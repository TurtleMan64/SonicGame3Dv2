...
public class RhinoTank extends Entity
{
	private static TexturedModel[] modelRhino = null;
	...
	public RhinoTank(Vector3f position) 
	{
		super(modelRhino, position);
	...
	private void moveMe()
	{
		xVel += (float)(0.7*Math.cos(Math.toRadians(-getRotY())));
		zVel += (float)(0.7*Math.sin(Math.toRadians(-getRotY())));
	}
	
	private void moveMeSlow()
	{
		xVel += (float)(0.08*Math.cos(Math.toRadians(-getRotY())));
		zVel += (float)(0.08*Math.sin(Math.toRadians(-getRotY())));
	}
	...
	public static void allocateStaticModels()
	{
		if (modelRhino == null)
		{
			modelRhino = ConvenientMethods.loadModel("Models/RhinoTank/", "RhinoTank");
		}
	}
	...
	public static void freeStaticModels()
	{
		if (modelRhino != null)
		{
			MainGameLoop.gameLoader.deleteModel(modelRhino);
			modelRhino = null;
		}
	}
}
